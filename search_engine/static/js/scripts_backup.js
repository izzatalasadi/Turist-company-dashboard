$(document).ready(function () {
    ...
    function deleteMessage(messageId) {
        $.post(`/delete_message/${messageId}`, function(response) {
            ...
            displayFlashMessages("Failed to delete the message", "danger");
        });
    }
...
if ('serviceWorker' in navigator) {
  ...
  function setupPDFViewer() {
    ...
    if (!canvas) return; // Exit if no canvas is present
    ...
    // Setup event listeners for PDF links
    document.querySelectorAll('.pdf-link').forEach(link => {
        link.addEventListener('click', function(e) {
            e.preventDefault();
            loadSelectedPDF(this.getAttribute('data-url'));  // Load and render the selected PDF
        });
    });
}

// Call setupPDFViewer when the DOM is fully loaded
